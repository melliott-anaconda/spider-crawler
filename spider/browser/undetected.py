...
def setup_undetected_webdriver(headless=True, retry_count=3):
    ...
    try:
        ...
        raise ImportError("undetected-chromedriver not installed. Install with: pip install undetected-chromedriver")
    ...
    for attempt in range(retry_count):
        try:
            ...
            print(f"Undetected ChromeDriver creation failed (attempt {attempt+1}/{retry_count}): {e}")
            time.sleep(2)
            
            if attempt == retry_count - 1:
                raise
    
    raise RuntimeError("Failed to create Undetected ChromeDriver after multiple attempts")