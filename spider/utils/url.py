...
def is_webpage_url(url, allowed_extensions=None):
    ...
    webpage_extensions = {'.html', '.htm', '.php', '.asp', '.aspx', '.jsp', '.do', '.xhtml', '.shtml'}
    ...
    excluded_extensions = {
        ...
        # Audio/Video
        '.mp3', '.wav', '.ogg', '.mp4', '.avi', '.mov', '.flv', '.wmv', '.webm', '.mkv',
        ...
        '.exe', '.dll', '.so', '.bin', '.apk', '.dmg', '.iso', '.msi',
...
def get_base_domain(url):
    ...
    # Get the last two parts of the domain (e.g. example.com)
    parts = netloc.split('.')
    if len(parts) > 2:
        # Handle special cases like co.uk, com.au, etc.
        tld_parts = parts[-2:]
        if parts[-2] in ['co', 'com', 'org', 'net', 'ac', 'gov', 'edu'] and len(parts[-1]) == 2:
            # It's a special case like co.uk
            if len(parts) > 3:
                return '.'.join(parts[-3:])
            return '.'.join(parts)
        else:
...
def get_page_links(driver, url, base_domain, path_prefix=None, allow_subdomains=False, allowed_extensions=None):
    """
            ...
            try:
                ...
                if href and not href.startswith('javascript:') and not href.startswith('#'):
                    ...
                    if allow_subdomains:
                        ...
                        domain_match = (base_domain_no_www == link_domain_no_www or 
...
def get_spa_links(driver, url, base_domain, path_prefix=None, allow_subdomains=False, allowed_extensions=None):
    """
    ...
    try:
        ...
        print(f"Found {len(links)} <a> links and {len(clickable_elements)} potential clickable elements on {url}")
                ...
                if element.is_displayed() and element.is_enabled():
                    ...
                    if new_url != original_url:
                        ...
                        if '#' in new_url:
                            ...
                            spa_url = f"{original_url}#{identifier.lower().replace(' ', '-')}"
...
def is_internal_link(url, base_url):
    ...
    # Compare domains (ignoring www prefix)
...
def get_url_parts(url):
    ...
    return {
        ...
        'domain': parsed.netloc.split(':')[0]  # Domain without port
    }
...
def sanitize_url(url):
    ...
    Returns:
        str: Sanitized URL
    """
    # Remove protocol-relative URLs that could leak info
    url = url.replace('\\\\', '')
    
    # Ensure the URL uses a common protocol
    parsed = urlparse(url)
    ...
    dangerous_patterns = [
        ...
        'chrome:', 'chrome-extension:', 'safari-extension:',
        'moz-extension:', 'about:', 'opera:', 'edge:'
    ]
