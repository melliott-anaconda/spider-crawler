...
class CrawlRateController:
    ...
    def register_response(self, response_data):
        ...
        with self.lock:
            ...
            if response_data.get("status") == "http_error":
                ...
                is_rate_limited = (
                    handling.get("action") == "throttle_and_retry"
                    or http_status == 429
                    or handling.get("rate_limited", False)
                )
                ...
                if is_rate_limited:
                    ...
                    if not self._recovery_mode:
                        ...
                        self._recovery_level = min(
                            3, self._recovery_level + 1
                        )  # Increase up to severe
                    ...
                    if self._recovery_mode:
                        ...
                        if self._consecutive_successes >= 20:
                            ...
                            if self._recovery_level == 0:
                                ...
                                self._throttling_active = False
                                print(
                                    f"Exiting recovery mode after {recovery_time:.1f}s and {self._consecutive_successes} consecutive successes"
                                )
            ...
            if len(self.recent_responses) > self.response_window_size:
                ...
                self.recent_responses = self.recent_responses[
                    -self.response_window_size :
                ]
    ...
    def should_adjust_now(self):
        ...
        with self.lock:
            ...
            if self._last_rate_limit_action is not None:
                cooldown_remaining = self._rate_limit_cooldown - (
                    now - self._last_rate_limit_action
                )
            ...
            if self._throttling_active and self.stats["rate_limited_requests"] > 0:
                if (
                    self._rate_limit_detected_time
                    and now - self._rate_limit_detected_time < 10
                ):
                    return True
            ...
            if self._recovery_mode:
                ...
                dynamic_interval = max(
                    5, self.adjustment_interval / (self._recovery_level + 1)
                )
                return now - self.last_adjustment_time >= dynamic_interval

            # Otherwise respect the standard adjustment interval
            return now - self.last_adjustment_time >= self.adjustment_interval and len(
                self.recent_responses
            ) >= min(5, self.response_window_size)
    ...
    def force_worker_reduction(self, reason="Emergency worker reduction"):
        ...
        with self.lock:
            ...
            print(
                f"FORCE_WORKER_REDUCTION called: current workers={original_workers}, current delay={original_delay:.2f}s"
            )
            print(
                f"Recovery level: {self._recovery_level}, Recovery mode: {self._recovery_mode}"
            )
            ...
            print(
                f"FORCED REDUCTION RESULT: workers {original_workers} → {self.target_workers}, delay {original_delay:.2f}s → {self.current_delay:.2f}s"
            )
            ...
            self.recent_responses = [
                r for r in self.recent_responses if not r.get("rate_limited", False)
            ]
    ...
    def adjust_rate_if_needed(self, force=False):
        ...
        if not force and self._last_rate_limit_action is not None:
            cooldown_remaining = self._rate_limit_cooldown - (
                time.time() - self._last_rate_limit_action
            )
            if cooldown_remaining > 0:
                return (
                    False,
                    self.target_workers,
                    self.current_delay,
                    f"In rate limit cooldown (for {int(cooldown_remaining)}s more)",
                )
        ...
        with self.lock:
            ...
            success_count = sum(
                1 for r in self.recent_responses if r.get("success", False)
            )
            rate_limited_count = sum(
                1 for r in self.recent_responses if r.get("rate_limited", False)
            )
            server_error_count = sum(
                1
                for r in self.recent_responses
                if not r.get("success", False) and 400 <= r.get("http_status", 0) < 600
            )
            ...
            if rate_limited_count > 0:
                ...
                changed, new_workers, new_delay, reason = (
                    True,
                    *self.force_worker_reduction(
                        f"Rate limiting detected ({rate_limited_count}/{total_responses})"
                    ),
                )
            ...
            # Check for high server error rate
                ...
                server_error_count / total_responses > 0.2
            ):  # More than 20% server errors
                ...
                reason = (
                    f"High server error rate ({server_error_count}/{total_responses}), "
            ...
            # If in recovery mode, be more conservative with scaling up
                ...
                if self._recovery_level >= 2:  # Moderate or Severe
                    ...
                    if (
                        ...
                        reason = (
                            f"Maintaining restrictive parameters during recovery "
                    ...
                    if success_rate >= 0.95 and self._consecutive_successes >= 15:
                        if self.target_workers < self.max_workers:
                            ...
                            self.current_delay = max(
                                self.min_delay, self.current_delay / 1.2
                            )
                            ...
                            self._throttling_active = False
                            reason = (
                                f"Exiting recovery mode after {recovery_time:.0f}s with "
                                f"high success rate ({success_rate:.1%})"
                            )
                    else:
                        reason = (
                            f"Maintaining parameters during light recovery "
            ...
            # Adjust based on success rate when not in recovery
                ...
                if self.target_workers < self.max_workers:
                    ...
                    reason = (
                        f"High success rate ({success_rate:.1%}) at max workers, "
                        f"decreasing delay to {self.current_delay:.1f}s"
                    )
                else:
                    reason = (
                        f"Maintaining parameters at optimal performance "
                        f"(success rate: {success_rate:.1%})"
                    )
                    return False, self.target_workers, self.current_delay, reason
                ...
                if self.target_workers > self.min_workers + 1:
                    ...
                    reason = (
                        f"Moderate success rate ({success_rate:.1%}) at near-minimum workers, "
                        f"increasing delay to {self.current_delay:.1f}s"
                    )
                ...
                self.target_workers = max(
                    self.min_workers,
                    self.target_workers - max(1, self.target_workers // 3),
                )
                self.current_delay = min(self.max_delay, self.current_delay * 1.5)
                ...
                reason = (
                    f"Low success rate ({success_rate:.1%}), reducing workers from {prev_target} to "
            ...
            changed = (
                original_workers != self.target_workers
                or abs(original_delay - self.current_delay) > 0.1
            )
    ...
    def get_statistics(self):
        ...
        with self.lock:
            ...
            stats["error_rate"] = (
                stats["server_errors"] + stats["client_errors"]
            ) / total
            ...
            if self._last_rate_limit_action:
                ...
                stats["cooldown_remaining"] = max(
                    0, self._rate_limit_cooldown - cooldown_elapsed
                )
    ...
    def from_checkpoint(self, checkpoint_data):
        ...
        with self.lock:
            try:
                ...
                print(
                    f"Restored rate controller from checkpoint: "
