...
import threading
import time
from multiprocessing import Manager, Value, Lock, Queue
...
class Spider:
    ...
    def __init__(self, start_url, keywords=None, output_file=None, 
                 max_pages=None, path_prefix=None, allow_subdomains=False,
                 content_filter=None, allowed_extensions=None, is_spa=False,
                 markdown_mode=False):
        """
        ...
        self.retry_queue = self.manager.Queue()  # Queue for URLs that need to be retried
        ...
        self.checkpoint_manager = CheckpointManager(
            checkpoint_file=f"{self.output_file}.checkpoint.json" if self.output_file else "spider_checkpoint.json"
        )
    ...
    def resume_from_checkpoint(self):
        ...
        """
        # First check for emergency checkpoint
        checkpoint_data = self.checkpoint_manager.check_for_emergency_checkpoint()
        
        # If no emergency checkpoint, try normal checkpoint
        ...
        try:
            ...
            self.to_visit[:] = pending_from_checkpoint + checkpoint_data.get('to_visit', [])
            ...
            if 'rate_controller' in checkpoint_data:
                with self.rate_controller.lock:
                    self.rate_controller.from_checkpoint(checkpoint_data.get('rate_controller', {}))
            ...
            print(f"Resumed from checkpoint: {len(self.visited)} visited URLs, {len(self.to_visit)} URLs to visit")
            print(f"Current settings: {self.target_workers.value} workers, {self.current_delay.value:.2f}s delay")
    ...
    def _handle_success_result(self, result):
        ...
        if self.markdown_mode and 'markdown_saved' in result:
            ...
            if keyword_results:
                ...
                if unique_results:
                    print(f"Found {len(unique_results)} unique keyword matches on {url}")
                    ...
                    print(f"All {len(keyword_results)} results from {url} were duplicates")
        ...
        if self.markdown_mode:
            category_counts = ', '.join(f"{k}: {v}" for k, v in self.markdown_stats.items())
    ...
    def _handle_http_error_result(self, result):
        ...
        print(f"HTTP error {http_status} for {url}: {handling.get('reason', 'Unknown error')}")
        ...
        if handling.get('action') == 'throttle_and_retry':
            ...
            with self.rate_controller.lock:
                ...
                changed, new_workers, new_delay, reason = self.rate_controller.adjust_rate_if_needed(force=True)
    ...
    def _process_retry_queue(self):
        ...
        while not self.stop_event.is_set():
            try:
                ...
                if delay_time > 0:
                    print(f"Scheduling retry for {url} in {delay_time:.1f}s (attempt {current_retries+1}/{self.max_retries})")
    ...
    def _check_scheduled_retries(self, retry_status):
        ...
        for url, info in retry_status.items():
            if info.get('scheduled', False) and info.get('retry_time', float('inf')) <= now:
    ...
    def _requeue_url(self, url, retry_id, retry_status):
        ...
        try:
            ...
            if current_status.get('scheduled', False) and current_status.get('retry_id') == retry_id:
    ...
    def _save_checkpoint(self, force=False):
        ...
        try:
            ...
            with self.rate_controller.lock:
                checkpoint_data['rate_controller'] = self.rate_controller.to_checkpoint()
            
            # Add markdown stats if in markdown mode
            if self.markdown_mode:
                checkpoint_data['markdown_stats'] = {k: v for k, v in self.markdown_stats.items()}
            
            # Save checkpoint
            saved = self.checkpoint_manager.save_checkpoint(checkpoint_data, force=force)
            
            if saved:
                print(f"Checkpoint saved: {self.pages_visited.value} pages visited, {len(self.to_visit)} URLs to visit")
    ...
    def _print_summary(self):
        ...
        if self.markdown_mode:
            ...
            for category, count in sorted(self.markdown_stats.items(), key=lambda x: x[1], reverse=True):
